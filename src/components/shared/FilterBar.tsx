      ...
      >
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2.5 w-full lg:w-auto">
          ...
          {onSortChange && sortOptions.length > 0 && (
            <div className="w-full sm:w-auto sm:min-w-38 shrink-0">
              <Select
                value={sortBy || undefined}
                onValueChange={(val) => onSortChange(val as SortType)}
              >
                <SelectTrigger className="w-full sm:w-auto min-w-38 h-10.5! border border-border/60 bg-background! text-sm font-medium focus:border-liminal-secondary/50 focus:ring-2 focus:ring-liminal-secondary/10 transition-all rounded-none pl-8 shadow-none relative flex items-center justify-start gap-2">
                ...
                <SelectContent
                  position="popper"
                  className="min-w-(--radix-select-trigger-width) w-max border border-border/60 bg-background/98 backdrop-blur-2xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)] rounded-none"
                >
                  {sortOptions.map((opt) => (
                    <SelectItem
                      key={opt.value}
